...
export default function Button({ variant = "primary", size = "md", className = "", children, ...rest }: Props) {
  ...
  const variants = {
    primary: "text-[var(--button-text)] shadow-md hover:shadow-lg focus:ring-[var(--theme-500)]",
  ...
  return (
    <button
      ...
      style={variant === 'primary' ? {
        background: 'linear-gradient(135deg, var(--button-bg), color-mix(in srgb, var(--button-bg) 80%, black))',
      } : undefined}
      {...rest}
    >
      {children}
