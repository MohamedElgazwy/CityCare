      ...
      <section className="mx-auto grid min-h-[calc(100vh-8rem)] max-w-7xl items-center gap-8 lg:grid-cols-2">
        <div>
          <span className="mb-5 inline-flex w-fit items-center rounded-full border border-white/70 bg-white/60 px-4 py-1 text-sm text-slate-700 backdrop-blur">
            منصة خدمات منزلية موثوقة في مدينة الشروق
          </span>
          <h1 className="max-w-3xl text-4xl font-bold tracking-tight text-slate-900 sm:text-6xl">
            تجربة رعاية منزلية مميزة مع فنيين محليين معتمدين.
          </h1>
          <p className="mt-6 max-w-2xl text-lg text-slate-600">
            اكتشف الخبراء، قارن الجودة، واحجز بثقة من خلال تجربة رقمية راقية.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/services"><Button>تصفح الخدمات</Button></Link>
            <Link href="/auth/register"><Button variant="secondary">إنشاء حساب</Button></Link>
          </div>
        </div>

        <Card className="relative overflow-hidden">
          <div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-cyan-200/70 blur-2xl" />
          <h2 className="text-2xl font-semibold text-slate-900">لماذا CityCare؟</h2>
          <ul className="mt-5 space-y-3 text-slate-700">
            <li>• حجز سريع وسلس بتجربة استخدام حديثة</li>
            <li>• أسعار واضحة وتقييمات تركز على الجودة</li>
            <li>• لوحات تحكم مختلفة حسب نوع المستخدم</li>
          </ul>
        </Card>
      </section>
    </main>
